(function () {
  ...
  const ROOM_CONFIGS = [
    { id: 0, name: 'Home', label: 'HOME', color: 0xff6644, emissiveColor: 0x661100, pos: [0, RADIUS, 0], type: 'top', contentType: 'home', icon: '⬡' },
    { id: 1, name: 'Projects', label: 'PROJECTS', color: 0x00ffcc, emissiveColor: 0x003322, pos: [0, 0, RADIUS], type: 'front', contentType: 'projects', icon: '◈' },
    { id: 2, name: 'Skills', label: 'SKILLS', color: 0x3388ff, emissiveColor: 0x001433, pos: [RADIUS, 0, 0], type: 'right', contentType: 'skills', icon: '⚡' },
    { id: 3, name: 'Experience', label: 'EXPERIENCE', color: 0x6644ff, emissiveColor: 0x110044, pos: [0, 0, -RADIUS], type: 'back', contentType: 'experience', icon: '◉' },
    { id: 4, name: 'Contact', label: 'CONTACT', color: 0xaa33ff, emissiveColor: 0x220044, pos: [0, -RADIUS, 0], type: 'bottom', contentType: 'contact', icon: '▼' },
    { id: 5, name: 'About', label: 'ABOUT', color: 0x7c8fff, emissiveColor: 0x101340, pos: [-RADIUS, 0, 0], type: 'left', contentType: 'about', icon: '◎' }
  ];
  ...
  function showRoomContentOverlay(cfg) {
    ...
    nav.innerHTML = `
      ...
      <button class="exit-btn" id="exitRoomBtn">✕ Exit Room <span style="opacity:0.6;font-weight:500;">· Space</span></button>`;
  ...
  function renderHomeContent(body, data, hex) {
    const p = data.personal || {};
    addCard(body, `<div style="font-family:'Cinzel',serif;font-size:1.8rem;color:#f3f3f6;line-height:1.3;margin-bottom:12px;">${esc(p.greeting||'')} ${esc(p.firstName||'')} ${esc(p.lastName||'')}</div><div style="font-size:0.9rem;color:rgba(243,243,246,0.6);line-height:1.7;max-width:600px;">${esc(p.heroSubtitle||'')}</div>`, hex, '0s');
    const stats = (p.stats || []);
    if (stats.length) {
      ...
      stats.forEach(s => {
        ...
        c.innerHTML = `<div style="font-family:'Cinzel',serif;font-size:2rem;color:${hex};">${esc(s.number)}</div><div style="font-size:0.62rem;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:1.5px;margin-top:6px;">${esc(s.label)}</div>`;
        grid.appendChild(c);
      });
      body.appendChild(grid);
    }
    addCard(body, `<div style="font-size:0.65rem;color:${hex};text-transform:uppercase;letter-spacing:2px;margin-bottom:12px;">Controls</div><div style="font-size:0.85rem;color:rgba(255,255,255,0.5);line-height:1.9;"><span style="color:rgba(255,255,255,0.75);">Arrow Keys / WASD</span> — Rotate the world<br><span style="color:rgba(255,255,255,0.75);">Click / Space</span> — Enter the facing room<br><span style="color:rgba(255,255,255,0.75);">Scroll / Swipe</span> — Navigate between rooms<br><span style="color:rgba(255,255,255,0.75);">Space</span> — Exit current room</div>`, hex, '0.12s');
  }
  ...
  function renderProjectsContent(body, data, hex) {
    ...
    projects.forEach((p, i) => {
      ...
      addCard(body, `<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;margin-bottom:12px;"><div><div style="font-family:'Cinzel',serif;font-size:1.05rem;color:#f3f3f6;margin-bottom:6px;">${esc(p.title)}</div><div style="margin-bottom:10px;">${tagsHtml}</div><div style="font-size:0.84rem;color:rgba(243,243,246,0.55);line-height:1.65;">${esc(p.description)}</div></div></div><div style="display:flex;gap:14px;margin-top:14px;">${p.liveUrl&&p.liveUrl!=='#'?`<a href="${p.liveUrl}" target="_blank" rel="noopener" style="font-size:0.7rem;font-weight:600;color:${hex};text-decoration:none;text-transform:uppercase;letter-spacing:1px;border:1px solid ${hex}44;padding:6px 14px;border-radius:4px;">Launch →</a>`:''}${p.sourceUrl&&p.sourceUrl!=='#'?`<a href="${p.sourceUrl}" target="_blank" rel="noopener" style="font-size:0.7rem;font-weight:600;color:rgba(255,255,255,0.5);text-decoration:none;text-transform:uppercase;letter-spacing:1px;border:1px solid rgba(255,255,255,0.08);padding:6px 14px;border-radius:4px;">Source</a>`:''}</div>`, hex, (i*0.07)+'s');
    });
  }
  ...
  function renderSkillsContent(body, data, hex) {
    ...
    cats.forEach((cat, ci) => {
      const itemsHtml = (cat.items||[]).map(s => `<div style="margin-bottom:14px;"><div style="display:flex;justify-content:space-between;font-size:0.82rem;margin-bottom:4px;"><span style="color:rgba(243,243,246,0.8);">${esc(s.name)}</span><span style="color:${hex};font-weight:600;">${s.level}%</span></div><div class="skill-bar-track"><div class="skill-bar-fill" style="width:${s.level}%;background:${hex};"></div></div></div>`).join('');
      addCard(body, `<div style="font-size:0.65rem;color:${hex};text-transform:uppercase;letter-spacing:2px;margin-bottom:16px;">${esc(cat.icon)} ${esc(cat.name)}</div>${itemsHtml}`, hex, (ci*0.08)+'s');
    });
  }
  ...
  function renderContactContent(body, data, hex) {
    ...
    addCard(body, `<div style="display:grid;gap:20px;"><div style="display:flex;align-items:center;gap:16px;"><span style="font-size:1.4rem;">📧</span><div><div style="font-size:0.62rem;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:1.5px;margin-bottom:3px;">Email</div><div style="font-size:0.9rem;color:#f3f3f6;">${esc(c.email||'')}</div></div></div><div style="display:flex;align-items:center;gap:16px;"><span style="font-size:1.4rem;">📍</span><div><div style="font-size:0.62rem;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:1.5px;margin-bottom:3px;">Location</div><div style="font-size:0.9rem;color:#f3f3f6;">${esc(c.location||'')}</div></div></div><div style="display:flex;align-items:center;gap:16px;"><span style="font-size:1.4rem;">🛡️</span><div><div style="font-size:0.62rem;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:1.5px;margin-bottom:3px;">Availability</div><div style="font-size:0.9rem;color:${hex};">${esc(c.availability||'')}</div></div></div></div>`, hex, '0s');
    ...
    formCard.innerHTML = `<div style="font-size:0.65rem;color:${hex};text-transform:uppercase;letter-spacing:2px;margin-bottom:20px;">Send a Message</div><div style="display:grid;gap:16px;"><div><label style="font-size:0.62rem;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:1px;display:block;margin-bottom:6px;">Name</label><input type="text" placeholder="Your name" style="width:100%;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:4px;padding:10px 14px;color:#f3f3f6;font-family:'Inter',sans-serif;font-size:0.85rem;outline:none;"></div><div><label style="font-size:0.62rem;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:1px;display:block;margin-bottom:6px;">Email</label><input type="email" placeholder="your@domain.com" style="width:100%;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:4px;padding:10px 14px;color:#f3f3f6;font-family:'Inter',sans-serif;font-size:0.85rem;outline:none;"></div><div><label style="font-size:0.62rem;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:1px;display:block;margin-bottom:6px;">Message</label><textarea rows="4" placeholder="Your message..." style="width:100%;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:4px;padding:10px 14px;color:#f3f3f6;font-family:'Inter',sans-serif;font-size:0.85rem;outline:none;resize:vertical;"></textarea></div><button onclick="event.preventDefault();this.textContent='Sent ✓';this.style.color='${hex}';this.style.borderColor='${hex}44';" style="align-self:start;background:${hex};color:#0d0d11;border:none;border-radius:4px;padding:11px 24px;font-family:'Inter',sans-serif;font-size:0.72rem;font-weight:700;text-transform:uppercase;letter-spacing:1px;cursor:pointer;">Send Message</button></div>`;
